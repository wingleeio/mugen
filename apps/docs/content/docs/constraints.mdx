...
The older escape hatch, `Portal` (an out-of-flow subtree measured as 0, the
base of `@wingleeio/mugen-ui`), is **deprecated**: overlay libraries portal
their own content, so the split-trigger pattern is no longer needed. Both keep
working, but new code should reach for `Escape`.
