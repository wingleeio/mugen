...
| ✅ Animated height (per row) | `Collapse` / `useMugenTween` tween the *committed* height: each frame re-measures the row (an O(log n) offset patch), so paint and offsets agree mid-flight. Whole-layout reflows (font settle, resize) still snap by design. |
