---
title: Recipes
description: Chat, accordions, deep links, and shrink-to-fit bubbles.
---

## AI chat with thinking & steps

A modern assistant transcript is a stress test for a virtualizer: every turn is a
different height, reasoning traces collapse and expand, and the newest turn keeps
growing as it streams. It maps cleanly onto mugen — each turn is a pure
`item → tree` using all three hooks. `useMugenState` collapses the “Thought for…”
trace (toggling re-measures just that row); `useMugenEffect` streams the final
turn's answer in word by word; and `stickToBottom` follows it down with a smooth
spring — scroll up to break free, scroll back to re-engage. It opens at the latest
turn (`initialScroll="bottom"`); hit **Replay** to watch it stream again, or expand
a reasoning trace and watch the list re-flow without losing your place:

<Showcase example="ai-chat" filename="ai-chat.tsx" />

## Bidirectional pagination

Top and bottom slots are measured like row content, so skeleton loaders can live
inside the scrollable coordinate system without throwing off row offsets.
`onTopReached` and `onBottomReached` fire once when the scroll position enters
their threshold, which is a clean fit for cursor pagination in either direction.
This audit-log feed opens in the middle of a loaded page; hit either edge and the
slot swaps in shimmering skeletons while the page resolves, then settles to the
exact height of the prepended/appended variable-height rows — no jump. Idle edges
show a soft hint, exhausted edges an end-cap:

<Showcase example="bidirectional-pagination" filename="pagination.tsx" />

## Rich markdown rows

[`@wingleeio/mugen-markdown`](https://www.npmjs.com/package/@wingleeio/mugen-markdown)
parses each row's markdown with [incremark](https://www.incremark.com/) and renders
it with mugen primitives — so headings, lists, fenced code, tables, and inline
**bold** / `code` / [links](https://www.incremark.com/) are all measured by the
walker. Off-screen rows have exact heights with no measure-on-mount shift, even
though every row is a different mix of blocks. Inline marks are styled through a
deep-partial `theme`; block nodes are overridable with a fully-typed `components`
map (here, `h1` gets an accent rule) — both authored from the same primitives, so
they stay measurable:

<Showcase example="mugen-markdown" filename="notes.tsx" />

## Tooltips, popovers & dialogs in rows

Overlays are the case that breaks a virtualizer: a tooltip or dropdown is a real
React component (hooks) whose popover is *portaled*, so it can't be measured
analytically — and the walker would throw if it tried. [`@wingleeio/mugen-ui`](https://www.npmjs.com/package/@wingleeio/mugen-ui)
splits the two halves. The **trigger** is measured like any primitive, so it
contributes its real height to the row; the **content** lives in a [`Portal`](/docs/constraints#out-of-flow-content-portal)
— measured as 0 and never walked — so it can be arbitrary React and never re-flows
the list when it opens. This 800-row directory gives every row a `Tooltip` (hover
the name), a `Popover` (React), a `Dropdown` (arrow-key menu), and a modal
`Dialog` (Details) — open any of them and the row heights never budge:

<Showcase example="overlays" filename="directory.tsx" />

Each widget is a compound component — `<Tooltip>` / `<Tooltip.Trigger>` /
`<Tooltip.Content>`, and likewise for `Popover`, `Dropdown`, and `Dialog`.
Positioning, dismissal, focus, and keyboard nav are handled for you; mugen core
stays a pure measurement engine.

## Chat / message list

Avatar on the left, author + message on the right. The avatar is a fixed-size box
(`width`/`height`); the text column grows into the remaining width — and every row
is a different height.

<Showcase example="chat" filename="chat-list.tsx" />

## Accordion rows

`useMugenState` controls whether the body renders. Toggling re-measures the row.
Click a question:

<Showcase example="accordion" filename="faq.tsx" />

## Deep link into a row

Open a specific row's state and scroll to it — the scroll lands pixel-exact
because the off-screen height is already known. Drive expansion from the item data
so it works before the row ever mounts:

```tsx twoslash
import { MugenVList, Text, VStack, useMugenVirtualizer } from '@wingleeio/mugen';

interface Thread {
  id: string;
  title: string;
  body: string;
  open: boolean; // expansion lives in the data
}

export function Threads({ threads, focusId }: { threads: Thread[]; focusId?: string }) {
  const list = useMugenVirtualizer({ items: threads });

  const openAndReveal = (id: string) => {
    // flip `open` for this thread in your own state, then:
    list.scrollToItem(id, { behavior: 'smooth', align: 'center' });
  };
  if (focusId) openAndReveal(focusId);

  return (
    <MugenVList
      instance={list}
      getKey={(t) => t.id}
      font="15px Inter"
      lineHeight={22}
      render={(t) => (
        <VStack gap={4} padding={12}>
          <Text font="600 15px Inter">{t.title}</Text>
          {t.open ? <Text>{t.body}</Text> : null}
        </VStack>
      )}
    />
  );
}
```

See [Scrolling](/docs/scrolling) for the full `scrollToItem` API.

## Shrink-to-fit bubbles

Chat bubbles that hug their text instead of filling the column:

```tsx twoslash
import { Text, VStack } from '@wingleeio/mugen';

interface Msg {
  id: string;
  text: string;
}

export function Bubble(msg: Msg) {
  return (
    <VStack padding={10}>
      <Text shrink>{msg.text}</Text>
    </VStack>
  );
}
```

`shrink` measures the text at its natural (tightest) width via pretext's
`naturalWidth`, and renders with `width: fit-content` so the browser shrink-wraps
to the same width.
