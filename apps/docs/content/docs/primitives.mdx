...
[`Collapse`](#collapse) for animated disclosures, and [`Escape`](#escape) for
...
```tsx twoslash
import { Text } from '@wingleeio/mugen';
...
```tsx twoslash
import { HStack, VStack, Text } from '@wingleeio/mugen';
...
```tsx twoslash
import { definePrimitive, Text } from '@wingleeio/mugen';
...
The animated disclosure. Toggling `open` tweens the row's **committed** height
between 0 and the children's measured natural height — offsets, scrollbar, and
paint agree at every intermediate frame, and rows below slide in lockstep. The
children measure like a `VStack`'s and render at full size inside a clipped box
pinned to the tween's current height.

```tsx twoslash
import { Collapse, Text } from '@wingleeio/mugen';
declare const item: { id: string; body: string };
declare const open: boolean;
// ---cut---
<Collapse id={item.id} open={open} duration={240}>
  <Text>{item.body}</Text>
</Collapse>;
```
...
| Prop | Type | Notes |
| --- | --- | --- |
| `id` | `string` | Required. Names the animation slot — unique within the row. |
| `open` | `boolean` | The target state; toggling animates. |
| `duration` | `number` | ms per toggle, default 200. `0` snaps. |
| `easing` | `MugenEasing` | Named curve or `t => progress`; default `ease-out`. |

Content that changes size *while* open (streaming text) snaps rather than
animates, so it composes with `stickToBottom`; `prefers-reduced-motion` snaps
everything. See [Animations](/docs/animations) for the model and `useMugenTween`.
...
declare function ShadcnTooltipButton(): React.ReactElement;
// ---cut---
<Escape height={32} width={38} className="flex items-center justify-center">
  <ShadcnTooltipButton /> {/* any React — hooks, raw divs, portals */}
</Escape>;
```
...
split. See [Constraints](/docs/constraints#escaping-the-walker-escape) for the
full contract.
