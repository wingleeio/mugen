...
```ts twoslash
import { useMugenVirtualizer } from 'mugen';
interface Item {
  id: string;
}
...
```ts twoslash
import { useMugenState, useMugenMemo, useMugenEffect } from 'mugen';
...
```ts twoslash
import type { MugenInstance } from 'mugen';
interface Item {
  id: string;
}
...
```ts twoslash
import type { ScrollToOptions } from 'mugen';
// ---cut---
